...
type PlayerLister interface {
	...
	List(selector labels.Selector) (ret []*tryapi.Player, err error)
...
// List lists all Players in the indexer.
func (s *playerLister) List(selector labels.Selector) (ret []*tryapi.Player, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*tryapi.Player))
	})
	return ret, err
}
...
type PlayerNamespaceLister interface {
	...
	Get(name string) (*tryapi.Player, error)
...
// List lists all Players in the indexer for a given namespace.
func (s playerNamespaceLister) List(selector labels.Selector) (ret []*tryapi.Player, err error) {
...
// Get retrieves the Player from the indexer for a given namespace and name.
func (s playerNamespaceLister) Get(name string) (*tryapi.Player, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(tryapi.Resource("player"), name)
	}
	return obj.(*tryapi.Player), nil
}
